...
def bspline_kernel_2d(sigma=[1, 1], order=2, asTensor=False, dtype=torch.float32, device='cuda'):
    ...
    :return:
    '''
    kernel_ones = torch.ones(1, 1, *sigma).to(dtype=dtype, device=device)
    kernel = kernel_ones.to(dtype=dtype, device=device)
    padding = torch.from_numpy(np.array(sigma)).to(dtype=int, device=device)
    with torch.no_grad():
