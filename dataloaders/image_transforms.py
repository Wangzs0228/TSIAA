...
class fourier_augmentation_reverse(object):
    def __init__(self, fda_beta=0.05):
        self.fda_beta = fda_beta
        
    def __call__(self, data):
        # horizontal flip with p = self.p
        data=data.transpose(2,0,1)[:,None,:,:]
        this_fda_beta = round(np.random.random() * self.fda_beta, 2)
        lowf_batch = data[::-1]
        fda_data = FDA_source_to_target_np(data, lowf_batch, L=this_fda_beta)
        return fda_data[:,0,:,:].transpose(1,2,0)

class fourier_augmentation_for_tensor(object):
    def __init__(self,fda_beta=0.05):
        # if fda_beta!=None:
        self.fda_beta=fda_beta
        # else:
