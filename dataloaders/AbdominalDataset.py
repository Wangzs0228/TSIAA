    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if self.location_scale is not None:
                ...
                comp = np.concatenate([GLA, LLA, curr_dict["img"],curr_dict["lb"]], -1)
                if self.transforms:
                    timg, raw_image, lb = self.transforms(comp, c_img=3, c_label=1, nclass=self.nclass, is_train=self.is_train,
                                              use_onehot=False)
        ...
        if self.tile_z_dim > 1:
            ...
            raw_image=raw_image.repeat( [ self.tile_z_dim, 1, 1] )
        ...
        sample = {"images": img,
                ...
                "index":index,
                "curr_dict": torch.from_numpy(np.transpose(np.float32(curr_dict["img"]), (2, 0, 1))),
                }
        return sample
